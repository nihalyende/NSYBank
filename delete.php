...
if (isset($_POST["Submit"])) {
		...
		$Amount = $_POST["Amount"];
		 global $ConnectingDB;
		$sql = "DELETE FROM customer WHERE id='$SearchQueryParameter'" ;
		$Execute=$ConnectingDB->query($sql);
		if($Execute) {
			echo '<script>window.open("view.php?id=Record Deleted Successfully","_self")</script>';  //Sendimg user back
		}
...
<nav class="navbar navbar-expand-md navbar-light fixed-top py-4 mb-5" id="main-nav">
	...
	?>
<div class="">
	<form class="mt-5 pt-5" action="delete.php?id=<?php echo $SearchQueryParameter; ?>" method="post">
		<fieldset>
